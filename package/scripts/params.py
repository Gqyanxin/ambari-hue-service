...
# Hue download url
# download_url = 'http://xxxxxxxxxxxxxxx/hortonworks/other/hue-4.11.0.tgz'
download_url = '`cat /etc/yum.repos.d/HDP.repo | grep "baseurl" | grep "HDP/centos" | awk -F \'=\' \'{print $2"hue/hue-4.11.0.tgz"}\'`'
...
Logger.info("config['clusterHostInfo']['hue_server_hosts']: {0} ".format(config['clusterHostInfo']['hue_server_hosts']))
ambari_server_hostname = config['clusterHostInfo']['hue_server_hosts'][0]
...
http_host = config['agentLevelParams']['hostname'].lower()
...
if security_enabled:
  ...
  zk_principal = config['configurations']['zookeeper-env']['zookeeper_principal_name']
...
if dfs_ha_enabled:
  ...
  hdfs_httpfs_host = config['configurations']['core-site']['hadoop.proxyuser.ambari.hosts']
  if len(hdfs_httpfs_host) > 1:
      hdfs_httpfs_host = config['configurations']['core-site']['hadoop.proxyuser.ambari.hosts']
  else:
...
hive_use_sasl = config['configurations']['hue-hive-site']['use_sasl']
hive_server_hosts =  default("/clusterHostInfo/hive_server_hosts", [])
Logger.info("hive_server_hosts: {0} ".format(hive_server_hosts))
...
if len(zookeeper_hosts) > 0:
  ...
  zookeeper_host_port = config['configurations']['hive-site']['hive.zookeeper.quorum']
  zookeeper_rest_url = ''
...
# configurations of Notebook Trino
notebook_integration_trino = config['configurations']['hue-desktop-site']['integration_trino']
notebook_integration_trino_url = config['configurations']['hue-desktop-site']['integration_trino_url']
...
desktop_auth_api_auth = config['configurations']['hue-auth-site']['api_auth']
