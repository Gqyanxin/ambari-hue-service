...
def download_hue():
  ...
  Execute('if grep -q "^hue" /etc/group; then echo "用户组hue已存在"; else groupadd hue; fi')
  Execute('if grep -q "^hue" /etc/passwd; then echo "用户hue已存在"; else useradd -d /home/hue -g hadoop hue; fi')
  #Execute('useradd -d /home/hue -g hadoop hue')
